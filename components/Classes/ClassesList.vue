<template>
  <b-card no-body>
      ...
      <template #cell(action)="row">
        <div class="td-action">
          <b-button
            variant="primary"
            class="mr-1 mb-1"
            @click="editClasses(row.item, row.index, $event.target)"
          >
            {{ $tc('Edit') }}
          </b-button>
          <b-button
    ...
    <EditClasses :classes="selectedClasses" />
    <AddClasses />
    <DeleteModel :execute-delelte="deleteClasses" />
  </b-card>
</template>
<script>
import AddClasses from '~/components/Classes/AddClasses.vue'
import EditClasses from '~/components/Classes/EditClasses.vue'
...
export default {
  ...
  methods: {
    ...
    editClasses(item, index, button) {
      this.selectedClasses = JSON.parse(JSON.stringify(item))
      this.$root.$emit('bv::show::modal', 'editclasses', button)
    },
    addClasses(button) {
      this.$root.$emit('bv::show::modal', 'addclasses', button)
    },
