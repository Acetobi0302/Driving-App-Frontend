<template>
  <b-modal id="editcar" ref="editcar" size="lg" hide-footer>
    <template #modal-title>
      <h2>{{ $tc('Update') }} {{ $tc('Car') }}</h2>
    </template>
    <card type="secondary" body-classes="px-lg-4 py-lg-2" class="border-0 mb-0">
      <ValidationObserver v-slot="{ invalid }">
        <b-form @submit.stop.prevent="editcar">
          <b-row>
            <b-col>
              <base-input
...
export default {
  ...
  methods: {
    ...
    async editcar() {
      ...
      //   this.form.user_id = null
      // }
      const response = await this.$store.dispatch('updateCar', this.form)
      if (response) {
        this.hideModal()
        this.$parent.getCars()
      }
